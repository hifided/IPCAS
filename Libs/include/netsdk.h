...
enum SDK_RET_CODE
{
	...
	H264_DVR_CATCHPIC_TIMEOUT			= -12001,			//抓图失败返回超时
...
typedef enum _SDK_CONFIG_TYPE
{
	...
	E_SDK_CFG_DIGITAL_ENCODE, 		//数字通道精简版编码配置---对应结构体SDK_EncodeConfigAll_SIMPLIIFY(需设置具体通道号)
	E_SDK_CFG_DIGITAL_ABILITY, 		//数字通道的编码能力---对应结构体SDK_DigitDevInfo（需设置具体通道号）
	...
	E_SDK_CFG_HUMAN_DETECTION,				//人形检测--SDK_HumanDectionCfgAll
	E_SDK_CFG_SET_RTC_TIME,					//设置RTC时间校正--SDK_SetRtcTime
	E_SDK_ABILITY_VENC_MAX_FPS,				//获取16CV300实际帧率--SDK_CaptureVencMaxFps_s
	E_SDK_CFG_KS_FaceDetect,				//旷视人脸识别配置--SDK_FaceKSDetectCfg
	E_SDK_CFG_KS_FaceFTPUpLoad, 			//旷视FTP上传图片类型--SDK_FaceKSPicUpLoadType
	E_SDK_CFG_HUMAN_DETECTIONDVR,			//DVR人形检测--SDK_HumanMotionDetectConfigAll
	E_SDK_CFG_AUDIO_FORMAT_TYPE,			//音频编码方式--SDK_AudioSupportType
	...
	E_SDK_CFG_LOG_STORAGE,					//日志存储-----SDK_LogStorage
	E_SDK_CFG_MULTICHANNEL_ENCODE,			//多通道编码--SDK_MultiChannelEncode
	E_SDK_ABILITY_MULTICHANNEL_ENCODE,		//多通道编码--SDK_MultiChannelEncodeAbility
	E_SDK_CFG_SET_FILEDATA,					//设置发送的文件数据（IE内部使用）
	E_SDK_OPERATION_NET_ONLINE_USR,			//ie端获取当前登录用户数--SDK_OnlineUsrList
	E_SDK_CFG_EMAIL_FOR_SEND_RESETCODE,		//重置密码功能中发送验证码的邮箱配置---SDK_NetEmailConfig
	E_SDK_CFG_FACE_DETECTION,				//人脸检测--SDK_FaceMotionDetectConfigAll
	E_SDK_CFG_KS_HTTP_COMMUNICATION,		//旷视http传输配置--SDK_KSHttpCommunication
	E_SDK_CFG_PSG_LEAD_SCREEN,				//IPC引导屏配置--SDK_LeadScreen
	E_SDK_CFG_INDICATOR_LIGHT,				//IPC外控灯配置--SDK_IndicatorLight
}SDK_CONFIG_TYPE;
...
enum SDK_EventCodeTypes
{
	...
	SDK_EVENT_CODE_HumanDetectAlarm,	//人形检测报警
	SDK_EVENT_CODE_FaceDetectAlarm,		//人脸检测报警
	SDK_EVENT_CODE_FaceRecognitionAlarm,//人脸识别
	SDK_EVENT_CODE_NR,
};
//! 编码配置的类型
...
typedef struct SDK_CONFIG_NET_COMMON_V2
{
	...
	char Version[NET_MAX_INFO_LEN];         //版本信息（定制的设备内容，一般设备没有）
	SDK_SYSTEM_TIME BuildDate;  			//版本日期（定制的设备内容，一般设备没有）
...
enum SDK_EncodeFunctionTypes
{
	...
	SDK_ENCODE_FUNCTION_TYPE_SMARTENCODE_DIGITAL,///Smart编码数字通道
	SDK_ENCODE_FUNCTION_TYPE_MULTICHANNEL,		///多通道编码
...
enum SDK_AlarmFucntionTypes
{
	...
	SDK_ALARM_FUNCTION_TYPE_NVR_HUMAN_DETECTION_NEW,///人形检测（NVR修正版）
	SDK_ALARM_FUNCTION_TYPE_IPC_PEA_IN_HUMAN_PED,	//加入周界拌线检测的人形检测(IPC)
	SDK_ALARM_FUNCTION_TYPE_FACE_DETECTION,			///人脸检测
	SDK_ALARM_FUNCTION_TYPE_HUMAN_DVR_SUPPORT_LEVEL,///DVR人形检测是否支持灵敏度	
...
enum SDK_NetServerTypes
{
	...
	SDK_NET_SERVER_TYPES_DUAL_ETHERNET,		//支持双网口
	SDK_NET_SERVER_TYPES_MidDAS,			//DAS集群功能受中心服务器控制
	SDK_NET_SERVER_TYPES_EMAIL_TLS,			//邮箱支持TLS加密
...
enum SDK_OtherFunction
{
	...
	SDK_OTHER_SUPPORT_ALARM_LINK_VOICE,		//支持报警联动语音提示功能
	SDK_OTHER_SUPPORT_ALARM_LINK_LIGHT,		//支持报警联动灯光提示功能
	...
	SDK_OTHER_SUPPORT_SHOW_H265X,			//显示H265X编码
	SDK_OTHER_SUPPORT_LOG_STORAGE_CTRL,		//日志存储控制
	SDK_OTHER_SUPPORT_ADMIN_CONTACT_INFO,	//支持设置管理员联系方式
	SDK_OTHER_SUPPORT_WARN_WEAK_PWD,		//显示弱密码提醒
	SDK_OTHER_SUPPORT_STRING_CHANGED_XPOE,	//支持通道模式显示为XPOE
	SDK_OTHER_SUPPORT_ALARM_VOICE_TIPSTYPE,	//支持报警提示音类型修改
	SDK_OTHER_SUPPORT_ACROSS_IPCTALK,		//支持前端IPC对讲
	SDK_OTHER_SUPPORT_SET_VOLUME,			//支持设置喇叭音量
	SDK_OTHER_SUPPORT_FACE_DETECT_V2,		//支持人脸检测
	SDK_OTHER_SUPPORT_FACEKS_HUMAN_DETECT,	//支持旷视人形检测
	SDK_OTHER_SUPPORT_FACEKS_HTTP_COMMUNICATION,//旷视http传输
	SDK_OTHER_SUPPORT_FACE_RECOGNITION,			//支持人脸识别 
	SDK_OTHER_SUPPORT_CUSTOMIZE_LPRECT,		//是否支持任意形状的车位
	SDK_OTHER_NO_SUPPORT_SAFETY_QUESTION,	//是否隐藏安全问题
	SDK_OTHER_SUPPORT_SYS_REMOTE_CTRL,		//支持系统远程控制
	SDK_OTHER_SUPPORT_ALARM_VOICE_TIP_INTERVAL,  //支持设备报警语音提示的循环播放的播放间隔
	SDK_OTHER_SUPPORT_SNAPSHOT_CONFIGV2,	//抓图配置V2
	SDK_OTHER_NR
};
...
typedef struct SDK_EventHandler
{
	...
	int		voiceTipInterval;		// 在报警期间报警语音提示播放间隔。0～3600秒，0表示不重复播放。
	int     voiceType;				// 和上面的bVoice对应，表示语音提示音类型，如枚举IAudioManager::DI_PLAY_TYPE
    bool    bShortMsg;              //发送短信
...
typedef struct H264_DVR_FINDINFO
{
	...
	char sEvent[32];		//自定义文件类型:SDK_File_Type为SDK_FILE_CUSTOM时需要传入，否则为空
	char sFileFormat[32];	//自定义文件格式:SDK_File_Type为SDK_FILE_CUSTOM时需要传入，否则为空
#ifndef OS_IOS
    H264_DVR_FINDINFO()
    {
        hWnd=0;
...
typedef struct SDK_CameraParamEx
{
	...
	int softPhotosensitivecontrol; //软光敏控制 0:光敏自动调节 1:软件自动调节 2:开启3:关闭
	int automaticAdjustment; // 自动调节档次 :  1~4
	int res[53];	//冗余
}SDK_CameraParamEx;
...
typedef struct SDK_Point
{
	int x;
	int y;
}SDK_Point;

typedef struct SDK_Size
{
	int w;
	int h;
}SDK_Size;
...
typedef struct SDK_IntelBrasSpecialInfo
{
	...
	char autoSearchNewFirm;				//是否开机自动搜索新版本
	char bIntelbrasProtocol;			//是否使能interbras云协议
	char rev2;
	int searchPort;						//搜索端口号
	int vvMinPort;						//vv 云最小端口
	int vvMaxPort;						//vv 云最大端口
	short  dyndnsUpdateTime;			//dyndns重新获取服务器时间,单位：min
	short  noipUpdateTime;				//no-ip重新获取服务器时间,单位：min
	int rev[10];
...
typedef struct SDK_MultiChannelEncodeAbility //结构体中带有容器，不能使用memset初始化整个结构体 sizeof[184 BYTE]
{
	unsigned int Compression;          ///支持的编码类型
	unsigned int BitRateControl;
	unsigned int uiMaxEncodePower;      ///< 支持的总编码能力
	unsigned int uiMaxBps;              ///< 支持的总码率大小Kbps
	unsigned int uiExImageSize;           ///< 支持的分辨率类型
...
typedef enum SDK_SENSOR_RESOUTION_TYPE
{
	...
	SDK_SENSOR_RESOLUTION_3M = 4,				//3M
	SDK_SENSOR_RESOLUTION_4M = 5,				//4M
	SDK_SENSOR_RESOLUTION_5M = 6,				//5M
	SDK_SENSOR_RESOLUTION_6M = 7,				//6M
	SDK_SENSOR_RESOLUTION_8M = 8,				//8M
	SDK_SENSOR_RESOLUTION_12M = 9,				//12M
...
{
	SDK_VIDEO_FORMAT vfFormat;	//视频格式定义 			
	bool bVideoEnable;			//是否开启多通道编码功能的使能
...
{
	SDK_RESTORE_PWD_VERIFYCODE_SEND_TO_APP = 0,				//重置密码用的验证码直接发送给扫码的APP
	SDK_RESTORE_PWD_VERIFYCODE_SEND_TO_ADMIN_ACCOUNT = 1,	//重置密码用的验证码只发给admin指定的邮箱或手机
}SDK_VerifyCodeRestorePwdType;

typedef struct SDK_PwdResetQsPair
{
	int questionIndex;					//问题序号，-1表示使用自定义问题
	char strAnswer[256];				//答案(已加密)
	char strQustion[256];				//自定义问题
}SDK_PwdResetQsPair;

typedef struct SDK_PasswordSafety
{
	bool bTipPageHide;					//是否隐藏密码为空时的提示页面
	int  iVerifyCodeRestorePwdType;		//通过验证码重置密码的方式，如枚举SDK_VerifyCodeRestorePwdType
	SDK_PwdResetQsPair PwdResetQs[4];	//安全问题和答案
	char resetMailAddr[64];				//用于接收超级密码的邮箱  
	char strPhone[64];					//手机号
	char resv[20];
}SDK_PasswordSafety;
...
//E_PGS_SPACE_STATE_ILLEGAL_PARKING 非法停车
//E_PGS_SPACE_STATE_ILLEGAL_LEFT 被左方车占用
//E_PGS_SPACE_STATE_ILLEGAL_RIGHT 被右方车占用
enum E_PGS_SPACE_STATE
{
	...
	E_PGS_SPACE_STATE_ILLEGAL_PARKING,
	E_PGS_SPACE_STATE_ILLEGAL_LEFT,
	E_PGS_SPACE_STATE_ILLEGAL_RIGHT,
...
typedef struct SDK_PLATE_DETECT		//车牌信息
{
	char acPlateCode[20];	//车牌号码，尾部有字符串结束符"\0" 
	char acPlateColor[4];	//车牌颜色，黄蓝黑白或"无"代表未知，字符串结束符"\0"
...
typedef struct SDK_InfoKSTailPic
{
	...
	int human_face_quality;	/*置信度*/
	int clusterid[5];		/*非0值，比如如果人脸和人形检测的检测结果是同一人，这个ID应该相等,目前只用到第0个*/
}SDK_InfoKSTailPic;

typedef struct SDK_FACE_DETECT		//人脸信息
{
	char sex;//性别
	char age;//年龄
	char resv[26];
...
typedef struct SDK_FR_TARGET_INFO	//人脸检测
{
	SDK_RECT_S stRect;
...
typedef struct SDK_LPR_TARGET_INFO	//车牌识别
{
	SDK_RECT_S stRect;
...
#define  Psg
typedef struct SDK_FR_TARGET_INFO_EX
{
	SDK_FR_TARGET_INFO m_FrTarget;
	SDK_InfoKSTailPic m_KSTailPic;
}SDK_FR_TARGET_INFO_EX;
...
{
	bool bEnable;
	SDK_Point vlocationrect[8];				//坐标描述，检测区域可以多个点,形状为不规则封闭图形	
}SDK_FaceKSDetectRoi;
...
{
	int nQuality;						//人脸置信度
	SDK_Size tMinFaceSize;					//最小人脸大小
	SDK_FaceKSDetectRoi	fRoi;
	int nBlurThreshold;
	SDK_FaceKSPoseAbsThreshold fFacePose;
	int capture_sen; //抓拍灵敏度，取值范围0-100
	int FilterThreshold;//后过滤,取值范围0-100
}SDK_FaceKSConstraint;

//推图策略
typedef struct SDK_FaceKSPushStrategy
{
	char smode[NET_NAME_PASSWORD_LEN];		//最优推图模式和定时推图"BEST"/"INTERVAL"
	int ninterval;//推图时间间隔
	int total;//推图张数
...
{
	bool	bShowBox;						//是否显示人脸框实时叠加
	bool	bShowFaceInfo;					//浏览界面 右侧的抓拍图片下方是否叠加人脸结构化信息字符
...
typedef struct SDK_FaceKSDetectCfg
{
	bool bEnable;							//true:开启，false:关闭
	int nfps;//帧率
	char chSnapMode[NET_NAME_PASSWORD_LEN];	//抓图模式，全抓模式和高质量抓拍模式"FULL"/"QUILITY"
	char FLAGS64[256];			//BASE64 加密
	SDK_FaceKSPushStrategy fFacePushStrategy; //推图策略，最优推图模式和定时推图
	SDK_FaceKSConstraint fConstraint;			//人脸检测参数
	int nExposure;							//人脸曝光值[1,100]
	SDK_FaceKSInfoDisPlay fDisplay;			//关于人脸信息显示
	bool bExpertEnable;						//是否是专家模式
	char CustomFPGA[1024];//fpga自定义配置
	char resv[16];		//保留字节
}SDK_FaceKSDetectCfg;
...
//人形检测
#define XM_IA_PD_NUM_MAX                (100)       /* 最大人形数 */
#define XM_IA_PD_RULE_NUM_MAX           (4)         /* 规则人形算法的最大规则数 */
...
typedef struct
{
	SDK_XM_IA_LINE_S stLine;                /* 绊线位置X1,Y1为起始点,X2,Y2为终止点(百分比表示0~8192对应0～100%) */
...
typedef struct
{
	...
	SDK_XM_IA_POINT_S astBoundaryPts[8];    /* 禁区描点位置(百分比表示0~8192对应0～100%) */
...
typedef struct
{
	SDK_XM_IA_RULE_LINE_S stRuleLine;       /* 绊线规则 */
	SDK_XM_IA_RULE_REGION_S stRuleRegion;   /* 禁区规则 */
}SDK_XM_IA_PD_RULE_U;
...
typedef struct
{
	...
	int eRuleType;							/* 规则类型 */
	SDK_XM_IA_PD_RULE_U unPdRule;           /* 具体规则 */
...
typedef struct
{
	SDK_XM_IA_PD_RULES_S stPdRules[XM_IA_PD_RULE_NUM_MAX];			/* 人形检测规则 */
...
typedef struct SDK_HumanDectionCfgAll
{
...
{
	bool bEnable;							// 智能人形检测使能(启用)
	int LoiterLatch;						//徘徊检测时间
	int	Level;								//灵敏度
	SDK_EventHandler hEvent;				// 智能人形检测联动 
...
}SDK_HumanMotionDetectConfigAll;

typedef struct SDK_SetRtcTime
{
...
typedef struct SDK_CaptureVencMaxFps_s
{
...
{
	int iEncodeType;   //编码类型--SDK_AudioEncodeTypes目前只支持SDK_AUDIO_ENCODE_G711_ALAW和SDK_AUDIO_ENCODE_G711_ULAW
...
//集群服务器，DAS到集群服务器获取空闲DAS服务器ip和端口号
typedef struct SDK_DASMidSerInfo
{
	bool enable;//DAS功能是否开启
	bool MidEnable;//中心服务器是否生效，如果生效，用中心服务器提供的ip和端口号，如果不生效，用设置的直接ip和端口号
	char serAddr[NET_NAME_PASSWORD_LEN];
	char resv[4];
}SDK_DASMidSerInfo;
...
{
	int iCtrlOtherDVR;        //控制DVR使能0:控制本地DVR, 1:控制远程DVR, 2:同时控制本地和远程DVR
	char sIP[64];			  //被控DVR IP
	char sUserName[64];		  //被控DVR用户名
	char sPassword[64];		  //被控DVR密码
	int iOtherRegionNo;       //被控DVR区域编号
	int iOtherLightNum;       //被控DVR区域对应指示灯个数
...
{
	bool	bStorageMemory;//日志存储到内存使能
	bool	bStorageFlash;//日志存储到flash使能
}SDK_LogStorage;
...
{
	bool bEnable;
	int iMode;		// 类型：1，group 0，bestface
	int iTime;
	bool bLevel;	//优先级
	int iMatchRate;	//匹配率
...
{	
	bool bHttpPushPicEnable; //http使能
...
{
	bool		bEnable;			//区域屏使能
	char		sLeadScreenIP[64];		//区域屏IP
	int			iLeadScreenPort;	//区域屏端口
	int			iLeadScreenESN;		//设备序号	
}SDK_LeadScreenCfg;

typedef struct SDK_LeadScreen
{
	bool		bOutEnable;			//车位输出使能
	int			iCarPortType[3];	//车位类型，0:普通车位,1:专用车位
	int			iCarPortOut[3];		//车位输出控制,0:禁止输出,1:允许输出
	SDK_LeadScreenCfg stLeadScreenCfg[5];//引导屏配置 
}SDK_LeadScreen;

//二维数组行LightEnable[0]内置灯，LightEnable[1]外置灯1，LightEnable[2]外置灯2，，LightEnable[3]外置灯3
//二维数组第1列接受控制，第2列车位1，第3列车位2，第4列车位3
...
/*
	nResult = 1表示发送完毕
	nResult = 2表示发送设备返回失败---具体错误：nError(101:未知错误;109查找失败，没有找到对应文件)
			注：NVR设备，若nError=0，则通过pErrorInfo获取具体出错的通道
	nResult = 3表示发送过程中出错
*/
...
/*
	...
	参数:
		...
		sPassword[in]:    密码
		lpDeviceInfo[out]:返回设备信息
		nType[in]:		  登陆类型，见enum SDK_LoginType（一般传0）
		sPwdHead[in]:	  加密前缀（一般定制使用）	
		error[out]:		  返回登陆错误码
		socketType[in]:	  登陆方式
	返回值:
		>0:	   成功
...
/*
	...
	参数:
		...
		lpInBuffer[in]:		 设置的json字符串
		lpInBufferlen[in]:	 设置的json字符串大小
		lpOutBuffer[out]:	 接收缓冲----接收设备返回的实际结果及内容
		waittime[in]:		 等待时间
	返回值:
		=1:成功
		<0:失败
...
/*
	...
	参数:
		sDVRIP[in]:			需要重置的设备IP
		wDVRPort[in]:		需要重置的设备端口号（一般为34567）
		strName[in]:		json名
		lpOutBuffer[out]:	接收缓冲----接收设备返回的实际结果及内容
		dwOutBufferSize[in]:接收缓冲的大小
		lpBytesReturned[out]:实际返回的大小
		socketType[in]:		连接类型------TCP：0
	返回值:
		true: 成功
		false:失败
*/
...
/*
	...
	参数:
		lLoginID[in]:			登陆句柄
		sFileName[in]:			要发送的文件数据
		cbSendFileData[in]:		回调发送进度
		dwUser[in]:				用户参数
	返回值:
		>0或<0: 成功
		=0:		失败
